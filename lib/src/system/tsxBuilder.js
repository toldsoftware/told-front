...
var TsxBuilder = (function () {
    ...
    TsxBuilder.prototype.subscribeActions = function (instance, content) {
        if (content.do) {
            ...
            var action_1 = content;
            if (instance.setActionLabel) {
                console.log("subscribeActions setActionLabel");
                instance.setActionLabel(action_1.label);
            }
            if (instance.setText && action_1 && action_1.scope && typeof action_1.scope.value === 'string') {
                console.log("subscribeActions setText");
                action_1.scope.subscribe(function (x) {
                    instance.setText(x);
                });
                instance.setText(action_1.scope.value);
            }
            if (!action_1.input) {
                console.log("subscribeActions setOnClick");
                instance.setOnClick(function () { return action_1.do(); });
            }
            else {
                if (instance.setOnTextChange) {
                    instance.setOnTextChange(function (x) { return action_1.do(x); });
                }
            }
        }
    };
